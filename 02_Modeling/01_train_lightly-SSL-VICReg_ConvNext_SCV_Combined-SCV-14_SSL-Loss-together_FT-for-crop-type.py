...
This script implements fine-tuning of the VICReg model with ConvNext backbone for crop-specific yield prediction.

The script:
1. Sets up data loading and preprocessing for each crop type
2. Loads pre-trained VICReg model with ConvNext backbone
...
def main() -> None:
    """
    Main fine-tuning function for VICReg with ConvNext backbone.
